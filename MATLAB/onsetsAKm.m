function [times, louds, types, banksigBank, banksigPeaktimeBank, peakPressBids, ...
      peaktimes, peakpresses, peakBgPresses,bankPresses] = onsetsAKm(signal, srate,...
    mininterval, loudthreshs, maskLevs, lboardLen)
...
%
% Files needed and call relations:
%   onsetsAK.m
%   onsetsAK/onsetComponents 
%   loudnOpt
%   pruneOutOnsets
%
% Reference: Klapuri, "Sound Onset Detection by Applying Psychoacoustic
%            Knowledge". ICASSP'99.
%
...
Fdec = 2000; % Hz. Decimation cutoff frequency
...
peakBgPresses = [];
peakPressBids = [];
bankPresses = zeros(Fcount,floor(loudtimefrac*length(signal)/srate));%ak:+1 omit
...
for bandid = 1:Fcount,
  if plotflag,
    disp(['  -band ', num2str(bandid), ' (',num2str(Fcount),')']);
  end;

  % -ONSET COMPONENTS AT THE BAND: detect and determine their parameters
  [peaktimesB, peakpressesB, peakBgPressesB, bankPressesB banksig banksigPeaktimes] =...
      onsetComponents( Bbank(bandid,:), Abank(bandid,:), signal, srate,...
      Io, timefrac, loudtimefrac,...
      Fdec, Bdec, Adec);
  
  
  % IO - collect banksigs to plot
  banksigBank = [banksigBank; banksig];
  
  % IO - collect peaktimes to plot
  %  banksigPeaktimeBank(bandid, :) = [banksigPeaktimeBank, banksigPeaktimes];
  if bandid == 4,
    banksigPeaktimeBank =  banksigPeaktimes;
  end
